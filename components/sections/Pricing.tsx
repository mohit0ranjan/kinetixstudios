...
export default function Pricing({
  ...
  return (
    <section
      id="pricing"
      className="py-20 md:py-32 relative bg-surface-1 overflow-hidden font-sans border-t border-black/5"
    >
      <div className="section-container relative z-10">
        {!hideHeader && (
          ...
          >
            <div className="flex justify-center mb-4">
              <span className="inline-flex items-center gap-2 bg-brand/10 border border-brand/20 px-4 py-1.5 rounded-full text-[11px] font-bold text-brand uppercase tracking-[0.2em]">
          ...
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`relative rounded-2xl overflow-hidden flex flex-col transition-all duration-500 ${
                plan.highlight
                  ? "bg-surface-dark border-[1.5px] border-brand/40 shadow-[0_20px_60px_-15px_rgba(255,106,0,0.25)] md:-translate-y-3"
                  : "bg-surface-0 border border-black/5 shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1"
              }`}
            >
              {plan.highlight && (
              ...
              <div className={`p-7 lg:p-8 flex-1 flex flex-col ${plan.highlight ? "" : "pt-7 lg:pt-8"}`}>
                ...
                <div className="mb-6">
                  ...
                  <p className={`text-[13px] font-medium ${plan.highlight ? "text-text-inverse/50" : "text-text-secondary"}`}>
                    {plan.tagline}
                  </p>
                </div>
                ...
                <ul className="space-y-3 flex-1 mb-8">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-2.5">
                      ...
                      />
                      <span className={`text-[13px] font-medium leading-snug ${plan.highlight ? "text-text-inverse/80" : "text-text-secondary"}`}>
                        {feature}
                      </span>
                    </li>
                  ))}
                  ...
                  <motion.a
                    ...
                    className={`w-full flex items-center justify-center gap-2.5 py-3.5 rounded-xl font-bold text-[14px] transition-all ${
                      plan.highlight
                        ...
                        : "bg-surface-1 text-text-secondary border border-black/5 hover:bg-surface-2 hover:text-text-primary"
                    }`}
                  >
                    <Phone size={16} />
                    Call Now
        ...
        >
          <p className="text-lg md:text-xl font-bold text-text-primary leading-relaxed mb-6">
